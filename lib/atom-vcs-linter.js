...
export function provideLinter() {
  return {
    ...
    lint(textEditor) {
      ...
      /*return parse('/users/micas/kgoetsch/Documents/ddic/alu_probeersel/src/tb.vcs:1:Error:testerrorlksjf\nd\n\n',
                   '(?<file>.+?):(?<line>[0-9]+):(?<severity>.+?):(?<excerpt>(?:.|[\n\r])+?)\n\n');
       return [{
         severity:'error',
         location:{
           file:'/users/micas/kgoetsch/Documents/ddic/alu_probeersel/src/tb.vcs',
           position: [[0, 0], [0, 1]]
         },
         excerpt:``+out[0].type
       }]*/
